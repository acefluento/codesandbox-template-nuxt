<template>
  <section id="contact" class="py-28 relative overflow-hidden">
    <!-- Background -->
    <div
      class="absolute inset-0 bg-gradient-to-br from-brand-navy via-brand-navy-2 to-brand-navy pointer-events-none"
    ></div>
    <div class="absolute inset-0 pointer-events-none">
      <div
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[500px] bg-brand-blue/5 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute top-0 right-1/4 w-[400px] h-[300px] bg-brand-red/4 rounded-full blur-3xl"
      ></div>
    </div>
    <div
      ...
      <div class="grid lg:grid-cols-2 gap-16 items-start">
        ...
        <div v-reveal="'fade-left'">
          <span class="section-label mb-6 inline-flex"
            >Book a strategy call</span
          >

          <h2
            class="text-4xl sm:text-5xl font-extrabold tracking-tight mt-4 mb-5 leading-tight"
          >
            If your business is getting attention<br class="hidden lg:block" />
            but not enough <span class="gradient-text">action</span> —<br
          ...
          <ul class="space-y-2.5 mb-8">
            ...
            >
              <div
                class="w-1.5 h-1.5 rounded-full bg-brand-red flex-shrink-0"
              ></div>
              {{ gap }}
            </li>
          </ul>
          ...
          <p class="text-white/40 text-sm mb-10 leading-relaxed max-w-xs">
            You'll leave with a clearer next move — whether we work together or
            not.
          </p>
          ...
          <div class="flex flex-wrap gap-5 text-white/45 text-sm">
            ...
            <a
              href="mailto:Acefluento@gmail.com"
              class="flex items-center gap-2 hover:text-white transition-colors"
            >
              <svg
                class="w-4 h-4 text-brand-blue"
        ...
        >
          ...
          <div
            v-else
            class="h-full min-h-[700px] p-8 bg-brand-navy-2/70 flex items-center justify-center"
          >
            <div class="max-w-sm text-center">
              <p class="text-white/70 text-base mb-5">
                Load the scheduler when you're ready to book.
              </p>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async loadCalendly() {
      ...
      const calendly = (
        window as Window & { Calendly?: { initInlineWidgets: () => void } }
      ).Calendly
