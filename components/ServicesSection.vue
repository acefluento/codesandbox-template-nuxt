<template>
  <section id="services" class="py-28 bg-brand-navy relative overflow-hidden">
    <div class="absolute inset-0 pointer-events-none">
      <div
        class="absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-48 bg-brand-blue/5 rounded-full blur-3xl"
      ></div>
    </div>
    ...
    <div class="relative max-w-7xl mx-auto px-6">
      ...
      <div v-reveal class="max-w-2xl mx-auto text-center mb-16">
        ...
        >
          Targeted deployments.<br /><span class="gradient-text"
            >One strategic goal.</span
          >
        </h2>
      ...
      <div v-stagger="100" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        >
          <!-- Icon -->
          <div
            class="w-11 h-11 rounded-xl bg-brand-blue/12 border border-brand-blue/20 flex items-center justify-center mb-5 group-hover:bg-brand-blue/20 transition-colors"
          >
            <svg
          ...
          <!-- Problem label -->
          <p
            class="text-white/30 text-xs font-semibold uppercase tracking-widest mb-2"
          >
            {{ service.problem }}
          </p>

          <h3 class="font-bold text-lg mb-2">{{ service.title }}</h3>
          <p class="text-white/50 text-sm leading-relaxed flex-1">
            {{ service.description }}
          </p>
        </div>
      </div>
