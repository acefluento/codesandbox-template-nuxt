<template>
  ...
  >
    <div
      class="max-w-2xl w-full bg-brand-navy-2 rounded-2xl border border-white/10 p-8 shadow-2xl"
    >
      <!-- Header -->
      <div class="flex items-center justify-between mb-6">
        ...
        <button
          class="w-8 h-8 rounded-xl bg-white/[0.05] hover:bg-white/10 flex items-center justify-center transition-colors"
          @click="$emit('close')"
        >
          <svg
      ...
      <div class="mb-5">
        <div class="flex items-center justify-between mb-2">
          <p class="text-white/50 text-xs uppercase tracking-wide font-medium">
            AI Draft Response
          </p>
          <span
            v-if="draft"
            class="text-xs px-2.5 py-1 rounded-full bg-brand-blue/15 text-brand-blue border border-brand-blue/20 capitalize"
          >
            {{ draft.tone }}
          </span>
        </div>
        <div
          v-if="draft"
          class="bg-brand-navy border border-white/10 rounded-xl p-5 text-sm text-white/80 leading-relaxed"
        >
          {{ draft.draft }}
        </div>
        <div
          v-else
          class="bg-brand-navy border border-white/10 rounded-xl p-5 text-sm text-white/30 italic"
        >
      ...
      <p class="text-white/30 text-xs mt-4">
        Review and edit before publishing. This is a draft — your voice and
        judgment are the final step.
      </p>
    </div>
  </div>
</template>
